...
- **Open detail.** Clear back to a match, then `click --text "Code / Chrome — 10分サマリ"`. Heading is that title. Section `Markdown 記憶` contains `## 作業サマリ` and an event mentioning `Code`. Back link `← タイムライン` returns to `/timeline`.
