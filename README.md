...
Git と Node.js 20 以降が必要です。ワンライナーはソースを `~/.local/share/shiftlog/src` に置き、この 1 台だけのランダムな API トークンを `.env` に書いてキーチェーンへ登録し、`pnpm setup:launchd` まで実行します。SQLite は `~/.local/share/shiftlog/shiftlog.db` に残ります。API はループバック（`127.0.0.1:8787`）のみで待ち受けます。
